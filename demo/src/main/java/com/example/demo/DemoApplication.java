...
public class DemoApplication {
	...
	public static void main(String[] args) {
		...
		boolean isIndexExists = validateIndexExists("sampleindex2");
		System.out.println("isIndexExists value is --> " + isIndexExists);

		// If Index does not exists, then create new index
		if (!isIndexExists) {
			createIndex("sampleindex2");
		}
	...
	private static boolean validateIndexExists(String index) {
		...
		try (RestHighLevelClient client = new RestHighLevelClient(
			...
			isIndexExists = client.indices().exists(new GetIndexRequest(index), RequestOptions.DEFAULT);
		} catch (Exception e) {
			isIndexExists = false;
